...
mod1 <- glmer(survival ~ froh_all10_cent + twin + sex + (1|sheep_year) + (1|birth_year) + (1|id) + (froh_all10_cent|age),
...
mod2 <- glmer(survival ~ froh_all10_cent * age_cent + froh_all10_cent * lamb_cent + twin + sex + (1|sheep_year) + (1|birth_year) + (1|id),
              family = binomial(link = 'logit'), data = annual_survival,
              control = glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
