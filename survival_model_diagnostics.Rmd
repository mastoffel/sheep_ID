...
Here, we will use ifferent diagnostic methods to show that the survival model
...
## Binned residuals
A usual model diagnostic is to plot residuals against fitted values, which is not very useful for logistic regression due to the discrete nature of the datapoints. An alternative is to divide the data into bins based on the fitted values and then plot the average residual versus the average fitted value for each bin. Here we divided the data into 100 bins of equal size. The lines are computed as 2*sqrt(p(1-p)/n) where n is the number of points per bin, in this case 14876/100 ~ 149, and represent the +-2 standard error bounds, "within one would expect 95% of the binned residuals to fall, if the model were actually true" (Gelman & Hill, 2007). 

```{r, warning=FALSE, message=FALSE, results='hide'}
performance::binned_residuals(mod1, n_bins =  100)
```

**While the result varies slightly depending on the number of bins, around 92-96% of our bins fall within the error bounds for our data, again indicating a good fit to the data.**
...
**Overall, the prediction accuracy varies around 80-81%. This shows that, on average,
the model correctly predicts the survival or death of new individual in 81% of cases.**
