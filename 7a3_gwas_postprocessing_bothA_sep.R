...
gwas_res <- bind_rows(all_gwas) %>% 
            ...
            separate(term, sep = 6, into = c( "allele","snp")) %>% 
            mutate(allele = str_remove(allele, pattern = "roh_")) %>% 
            mutate(allele = str_remove(allele, pattern = "_"))

gwas_res %>% 
        #
        pivot_wider(names_from = allele, values_from = c("estimate", "p.value")) %>% 
        mutate(interesting = ifelse((p.value_0 < 0.000005) | (p.value_2 < 0.000005), 1, 0)) %>% 
        filter(interesting == 1) %>% 
        #filter(estimate_1 < 0 & estimate_2 <0) %>% 
        #filter(estimate_1 > -10) %>% 
        ggplot(aes(estimate_0, estimate_2)) +
        geom_point() +
