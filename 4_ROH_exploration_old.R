...
ggsave("figs/roh_genome.jpg", p_final, width = 7, height = 6)
...
df %>% 
        ...
        geom_rect(data=shade, aes(xmin=min, xmax=max, ymin=0, ymax=num_ind*2 + 1), 
                  alpha=0.7, fill = "#f7f7f7") +
        geom_hline(data = yax, aes(yintercept = yax), color = "grey", size = 0.1) +
