...
plot_model(mod_lme4, type = "pred", terms = c("froh_all10[all]", "life_stage"), 
           show.values = TRUE, show.data = TRUE)

library(Hmisc)
annual_survival %>% 
        mutate(binned_froh = cut2(froh_all10, g = 10)) %>% 
        group_by(life_stage, binned_froh) %>% 
